...
comptime USE_BROADCAST_WAKE = USE_COOPERATIVE_FAA or get_defined_bool[
    "MOSTREAM_COOPERATIVE_BROADCAST", False
]()
...
struct Scheduler[*Ts: NodeTrait]:
    ...
    var descriptors: Pointer[ActorDescriptor, MutUntrackedOrigin]
    var stage_offsets: Pointer[Int, MutUntrackedOrigin]
    var batch_size: Int
    var activations: UInt64
    var input_parks: UInt64
    var output_parks: UInt64

    # constructor
    def __init__(out self, mut nodes: Tuple[*Self.Ts], batch_size: Int = 1) raises:
        ...
        self.descriptors = unsafe_alloc[ActorDescriptor](self.total_actors)
        self.stage_offsets = unsafe_alloc[Int](self.num_stages + 1)
        ...
        comptime for i in range(len(Self.Ts)): # i is the stage index
             self.stage_offsets[unsafe_offset=i] = flat_id
             var parDegree = nodes[i].parallelism()
             for j in range(parDegree): # j is the node index within the stage
                var descriptor = ActorDescriptor(stage_idx=i, replica_idx=j, flat_id=flat_id)
                self.descriptors[unsafe_offset=flat_id] = descriptor
                self.ready_queue[].push(descriptor)
                flat_id += 1
        self.stage_offsets[unsafe_offset=self.num_stages] = flat_id
    ...
    def __deinit__(deinit self):
        self.descriptors.unsafe_free()
        self.stage_offsets.unsafe_free()
    ...
    def start(mut self, mut nodes: Tuple[*Self.Ts], n_workers: Int, mut pinning_handler: Pinning):
        var tg = TaskGroup()
        var metrics = unsafe_alloc[UInt64](n_workers * 3)
        for i in range(0, n_workers):
            tg.create_task(self.worker_loop(nodes, pinning_handler.get_next_core_id(), pinning_handler, metrics.unsafe_offset(i * 3)))
        tg.wait()
        for i in range(n_workers):
            self.activations += metrics[unsafe_offset=i * 3]
            self.input_parks += metrics[unsafe_offset=i * 3 + 1]
            self.output_parks += metrics[unsafe_offset=i * 3 + 2]
        metrics.unsafe_free()
        ...
        comptime for i in range(len(Self.Ts)):
            if actor.stage_idx == i:
                for _ in range(self.batch_size):
                    var result = nodes[i].actor_ref(actor.replica_idx)[].process()
    ...
    def wake_one_input_waiter(mut self, comm_idx: Int):
        comptime if USE_BROADCAST_WAKE:
            self.wake_stage_waiters(comm_idx + 1, ActorStatus.BLOCKED_INPUT)
            return
        while True:
    ...
    def wake_one_output_waiter(mut self, comm_idx: Int):
        comptime if USE_BROADCAST_WAKE:
            self.wake_stage_waiters(comm_idx, ActorStatus.BLOCKED_OUTPUT)
            return
        while True:
    ...
    # FAA tickets belong to particular actors. Broadcast via atomic states,
    # without reading operation fields while an actor may still be parking.
    def wake_stage_waiters(mut self, stage_idx: Int, blocked_state: UInt64):
        # Pair with the parking-side fence so publication and registration
        # cannot both be missed on weakly ordered architectures.
        fence[ordering=Ordering.SEQUENTIAL]()
        for flat_id in range(self.stage_offsets[unsafe_offset=stage_idx], self.stage_offsets[unsafe_offset=stage_idx + 1]):
            if self.actor_states[unsafe_offset=flat_id].load[ordering=Ordering.ACQUIRE]() != blocked_state:
                continue
            var expected = blocked_state
            if self.actor_states[unsafe_offset=flat_id].compare_exchange[
                success_ordering=Ordering.ACQUIRE_RELEASE,
                failure_ordering=Ordering.RELAXED,
            ](expected, ActorStatus.READY):
                self.ready_queue[].push(self.descriptors[unsafe_offset=flat_id])
    ...
    def park_on_input_or_ready(mut self, mut nodes: Tuple[*Self.Ts], actor: ActorDescriptor) raises:
        ...
        if not self.actor_states[unsafe_offset=actor.flat_id].compare_exchange[
            ...
            return
        comptime if USE_BROADCAST_WAKE:
            # Publish BLOCKED before checking again: an event either wakes
            # us through the state scan or is observed by this recheck.
            fence[ordering=Ordering.SEQUENTIAL]()
            ...
            elif self.actor_input_is_closed(nodes, actor):
                self.mark_blocked_ready(actor, ActorStatus.BLOCKED_INPUT)
            self.set_not_busy(actor)
            return
    ...
    def notify_after_blocked_input(mut self, mut nodes: Tuple[*Self.Ts], actor: ActorDescriptor) raises:
        # Earlier steps in this batch may have consumed input before the last
        # step blocked; publish that capacity change as well.
        if self.batch_size > 1 and actor.stage_idx > 0:
            self.wake_one_output_waiter(self.input_wait_queue_idx(actor))
        # the actor might have produced output(s), data may be available downstream
        if actor.stage_idx < self.num_stages - 1: # if it is not a sink
            self.wake_downstream_input_waiters(actor)
    ...
    # main worker loop
    async def worker_loop(mut self, mut nodes: Tuple[*Self.Ts], core_id: Int, mut pinning_handler: Pinning, metrics: Pointer[UInt64, MutUntrackedOrigin]):
        try:
            var activations: UInt64 = 0
            var input_parks: UInt64 = 0
            var output_parks: UInt64 = 0
            ...
            while self.done_count[].load[ordering=Ordering.ACQUIRE]() < UInt64(self.total_actors):
                ...
                if result == ActorStatus.READY:
                    ...
                    self.mark_done(actor)
            metrics[] = activations
            metrics[unsafe_offset=1] = input_parks
            metrics[unsafe_offset=2] = output_parks
        except e:
