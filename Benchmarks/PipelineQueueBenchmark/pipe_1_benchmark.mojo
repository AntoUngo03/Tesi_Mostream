# Timed, silent equivalent of Tests/test_pipe_1.mojo.
...
from MoStream import Pipeline, StageKind, StageTrait, seq
...
struct SecondStage(StageTrait):
    ...
    def compute(mut self, var input: Int) -> String:
        return String("Valore " + String(input + 1))
...
def expected_checksum(elements: Int) -> UInt64:
    ...
    for value in range(1, elements + 1):
        checksum += 7 + decimal_digits(value + 1)
    return checksum
...
def run_once(elements: Int) raises:
    ...
    var expected_count = UInt64(elements)
    ...
    var transfers = expected_count * 2
    var throughput = (
        Float64(transfers) / Float64(elapsed_ns) * 1000.0
    )
    print(
        "PIPE_RESULT test=pipe_1 backend=", backend,
        " time_ms=", elapsed_ms,
    ...
    if not valid:
        raise Error("pipe_1 benchmark correctness failure")


def main() raises:
    var args = argv()
    if len(args) != 2:
        print("Usage: pipe_1_benchmark <elements>")
