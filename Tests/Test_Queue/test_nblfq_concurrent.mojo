...
def run_case(capacity: Int, producers: Int, consumers: Int, use_try: Bool) raises:
    ...
    var queue = NBLFQQueue[Payload](capacity)
    var finished = Atomic[DType.uint64](0)
    var last = unsafe_alloc[Int](producers)
    for i in range(producers):
        last[unsafe_offset=i] = -1
        ...
        if thread_id < producers:
            ...
            while True:
                ...
                if payload.id < 0 or payload.id >= total:
                    ...
                    if consumers == 1:
                        var producer = payload.id // messages
                        var sequence = payload.id % messages
                        if sequence != last[unsafe_offset=producer] + 1:
                            _ = invalid.fetch_add[ordering=Ordering.RELAXED](1)
                        last[unsafe_offset=producer] = sequence
    ...
    if not valid:
        print("FAIL: NBLFQ exact-once", capacity, producers, consumers, use_try)
        exit(1)
    print("PASS: NBLFQ exact-once capacity/P/C/try", capacity, producers, consumers, use_try)
...
def main() raises:
    ...
    run_case(4, 1, 8, True)
    run_case(4, 8, 1, True)
    print("PASS: NBLFQ concurrent suite with owned string payloads")
